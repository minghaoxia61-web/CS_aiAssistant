...
- **自实现 BM25**：完整 IDF/TF/k1/b 公式 + 中英文分词（英文单词 + 中文 2-gram/3-gram + 单字知识点精确命中）
...
- **RAG 检索评测**：从课件片段自动建立定位问题，计算 Hit@1、Hit@3、Hit@5、MRR、资料覆盖率与回答引用覆盖率
- **页码级引用与答案评测**：PDF/PPT 引用可定位到页码或幻灯片，并评估证据一致性、引用有效率、拒答准确率与幻觉风险
- **AI 语义知识图谱**：在课件标题关系之外生成概念、前置与关联边，每个节点保留证据、置信度并允许人工修正
- **长期学生模型**：持续记录正确率、耗时和改答案次数，区分知识缺口、粗心、概念混淆与遗忘，预测遗忘风险和下一题难度
- **自适应学习计划**：根据长期掌握轨迹生成错题复习、专项练习和预习任务
- **跨页面联动**：知识节点可直接进入资料问答或专项测验，自动携带对应课件、章节和难度
- **教师工作台**：创建班级邀请码、发布任务、查看班级指标和知识点热力图
...
学习实验室使用 Bayesian Knowledge Tracing 对每个知识点维护可解释的掌握概率，而不把加权正确率直接当成概率。

- 参数：初始掌握 `P(L0)`、学会 `P(T)`、猜对 `P(G)`、失误 `P(S)` 和可选的按日遗忘率。
...
SOLO Agent 的主动提醒之外，学习实验室还提供一条可持久、可恢复的学习干预状态机：

```text
observe → diagnose → ground → intervene → verify → update → schedule
```

- `observe`：只读取当前科目的资料、测验、错题和 BKT 状态。
- `diagnose`：结合下一题答对概率与错因分类，选择当前干预目标。
- `ground`：在同一科目中检索课件证据；无证据时进入 `blocked`，不生成确定性讲解。
- `intervene`：根据知识缺口、概念混淆、粗心或遗忘，选择证据讲解、对比讲解、限时验证或错题复习。
- `verify`：必须获得计划创建之后的新作答或复习证据。
- `update`：验证完成后才允许更新 BKT 掌握概率。
- `schedule`：使用错题的间隔复习调度给出下一次复习时间。

页面会展示每个状态的推理依据、课件定位、执行状态和 BKT 前后变化。Agent 不会自动发送对话、自动完成测验或在缺少验证时宣称用户已掌握。
...
回退资料仍可完整执行分块检索、BKT 更新、Agent 证据定位和验证闭环。自动测试会从演示资料一直跑到 Agent `complete` 状态，防止“页面能打开但演示链路断了”。
...
# 只读审计所有可达 Git 历史，仅输出提交和文件名
npm run audit:secrets-history
```

历史审计不会打印密钥原文。如果有命中，应先在服务商处轮换/撤销密钥，再与所有协作者协调 Git 历史重写；仅从最新代码删除密钥并不能清除历史泄漏。
...
1. 创建 Supabase 项目，在 SQL Editor 中按顺序执行 `supabase/migrations/` 下的三个迁移文件
2. 启用 Email 登录
3. 配置 `VITE_SUPABASE_URL` 与 `VITE_SUPABASE_PUBLISHABLE_KEY`
4. 在个人信息页使用邮箱魔法链接登录，可开启自动同步、恢复历史版本并获得冲突保护

数据库已启用 RLS，每位用户只能访问自己的学习快照；教师只能管理自己创建的课程与班级。请勿在前端环境变量中使用 `service_role` key。
